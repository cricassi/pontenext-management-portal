...
test("registry: every business screen inactive; unique optional pairs match SQL exactly", () => {
  const pairs: string[] = [];
  assert.equal(VISIBILITY_SCREEN_KEYS.length, 42);
  for (const [key, screen] of Object.entries(FIELD_VISIBILITY_REGISTRY)) {
    assert.equal(screen.integrated, false);
...
test("save rejects missing, unknown, required fields, truthy strings and technical metadata", () => {
  ...
  for (const key of VISIBILITY_SCREEN_KEYS) assert.throws(() => assertVisibilityIntegrated(key), errorCode("not_integrated"));
...
function serviceFixture(options: { role?: string; denied?: boolean; readError?: boolean; truncated?: boolean; integrated?: boolean; writeError?: string } = {}) {
  ...
  const service = loadModule<Service>("src/services/field-visibility.service.ts", {
    ...
    "@/services/supabase.service": { getSupabaseServerClientOrThrow: async () => { events.push("client"); return { from: (table: string) => { events.push(table); assert.equal(table, "ui_field_visibility"); return builder; } }; } },
...
test("anonymous denied before any configuration query", async () => {
  ...
  await assert.rejects(fixture.service.saveFieldVisibility(saveInput()), /NEXT_REDIRECT/);
  await assert.rejects(fixture.service.resetFieldVisibility("members.edit"), /NEXT_REDIRECT/);
...
test("ordinary admin cannot write; inactive integration rejects super_admin without queries", async () => {
  for (const role of ["admin", "super_admin"]) {
    ...
    await assert.rejects(fixture.service.saveFieldVisibility(saveInput()), errorCode(code));
    await assert.rejects(fixture.service.resetFieldVisibility("members.edit"), errorCode(code));
...
test("DB failures and truncated reads default visible and forbid a write", async () => {
  for (const options of [{ readError: true }, { truncated: true }]) {
    ...
    await assert.rejects(fixture.service.saveFieldVisibility(saveInput()), errorCode("unavailable"));
    assert.equal(fixture.written(), undefined);
  }
});

test("isolated enabled fixture: one bulk PK upsert, fresh read, soft reset and no retry", async () => {
  const fixture = serviceFixture({ integrated: true });
  await fixture.service.getFieldVisibility(["members.edit"]);
  await fixture.service.saveFieldVisibility(saveInput());
  assert.equal(fixture.events.filter((value) => value === "select").length, 2);
  assert.equal(fixture.events.filter((value) => value === "upsert").length, 1);
  assert.deepEqual((fixture.written() as { conflict: unknown }).conflict, { onConflict: "id" });
  await fixture.service.resetFieldVisibility("members.edit");
  assert.equal((fixture.written() as { updated_by: string }).updated_by, "verified-admin");
  assert.ok((fixture.written() as { archived_at: string }).archived_at);
  const conflict = serviceFixture({ integrated: true, writeError: "23505" });
  await assert.rejects(conflict.service.saveFieldVisibility(saveInput()), errorCode("conflict"));
  assert.equal(conflict.events.filter((value) => value === "upsert").length, 1);
});

test("actions sanitize errors, preserve auth redirects and revalidate only settings", async () => {
  ...
  assert.equal((await actions.saveFieldVisibilityAction(saveInput())).ok, true);
  assert.equal((await actions.resetFieldVisibilityAction("members.edit")).ok, true);
  assert.deepEqual(invalidations, ["/settings/field-visibility", "/settings/field-visibility"]);
  failure = new Error("secret-token-db-error");
  assert.doesNotMatch((await actions.saveFieldVisibilityAction(saveInput())).message, /secret|token|db-error/);
  failure = new Error("NEXT_REDIRECT");
  await assert.rejects(actions.saveFieldVisibilityAction(saveInput()), /NEXT_REDIRECT/);
});

test("catalog renders controls disabled, no readonly/group/import, mobile and desktop views", () => {
  ...
  const snapshot = resolveVisibilityRows(["members.create"], []);
